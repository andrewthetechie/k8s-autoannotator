...
def add_dns(service):
    ...
    elif "external-dns.alpha.kubernetes.io/altname" in service.metadata.annotations:
        logging.debug(
            "Service named %s has an altname of %s supplied",
            service.metadata.name,
            service.metadata.annotations["external-dns.alpha.kubernetes.io/altname"],
        )
        dns_name = (
            f"{service.metadata.service.metadata.annotations['external-dns.alpha.kubernetes.io/altname']}."
            f"{CLUSTER_FDQN}"
        )

    patch = {
        "metadata": {
            "annotations": {"external-dns.alpha.kubernetes.io/hostname": dns_name}
        }
    }
    ...
    logging.debug(
        v1.patch_namespaced_service(
            name=service.metadata.name, namespace=service.metadata.namespace, body=patch
        )
    )
    ...
    for service in v1.list_service_for_all_namespaces(watch=False).items:
        ...
        if service.spec.type == "LoadBalancer":
            ...
            if (
                ...
                logger.debug(
                    "Service %s does not have dns setup", service.metadata.name
                )
                # if we've set nodns annotation, don't setup annotation
                if (
                    "external-dns.alpha.kubernetes.io/nodns"
                    ...
                    logging.debug(
                        "Service named %s has nodns annotation set",
                        service.metadata.name,
                    )
                    continue
                logger.debug("Adding DNS for service named %s", service.metadata.name)
